...
1. 后端接口约束：所有小语相关接口路径必须以/api/xiaoyu/开头，返回格式统一为{ success: boolean, data?: T, error?: string }；
1. 资源引用约束：前端小语动画 / 音效必须从/assets/animations/、/assets/sounds/路径引用，不使用绝对路径；
1. 错误处理约束：前端用react-hot-toast提示错误，后端用AppError异常类 +error.middleware.ts统一捕获。
