    ...
    >
      <div className="flex items-center gap-3 mb-5">
        <div className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-md ${
          ...
          <BookOpen size={18} className={isCyberpunk ? 'text-black' : 'text-white'} />
        </div>
      ...
      <div className="flex justify-center mb-5">
        <div className="relative w-24 h-24">
          <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
            <circle cx="50" cy="50" r="42" fill="none" stroke={isCyberpunk ? 'rgba(0,240,255,0.1)' : 'rgba(16,185,129,0.1)'} strokeWidth="8" />
            <circle
              cx="50"
              cy="50"
              r="42"
              fill="none"
              stroke={`url(#${isCyberpunk ? 'cyberGrad' : 'greenGrad'})`}
              strokeWidth="8"
              strokeLinecap="round"
              strokeDasharray={`${progress * 2.64} ${264 - progress * 2.64}`}
              style={{ filter: isCyberpunk ? 'drop-shadow(0 0 6px rgba(0,240,255,0.5))' : 'drop-shadow(0 0 6px rgba(16,185,129,0.4))' }}
            />
            <defs>
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <span className={`text-2xl ${isCyberpunk ? 'text-[#00f0ff]' : 'text-emerald-600'}`} style={{ fontWeight: 700 }}>{progress}%</span>
          </div>
        </div>
      </div>
