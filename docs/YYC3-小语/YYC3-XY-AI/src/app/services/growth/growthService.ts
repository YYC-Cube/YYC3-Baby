...
export interface GrowthStatisticsData {
  totalRecords: number;
  milestones: number;
  culturalActivities: number;
  attachmentCounts: { photos: number; videos: number };
  growthAreas: {
    category: string;
    count: number;
...
class GrowthService {
  ...
  async getGrowthTimeline(): Promise<GrowthTimelineData> {
    return new Promise<GrowthTimelineData>((resolve) => {
      setTimeout(() => {
        resolve({
          phases: {
  ...
  async getGrowthStatistics(): Promise<GrowthStatisticsData> {
    return new Promise<GrowthStatisticsData>((resolve) => {
      setTimeout(() => {
        resolve({
  ...
  async addGrowthRecord(record: Record<string, unknown>): Promise<void> {
    return new Promise((resolve) => {
      setTimeout(() => {
        const newRecord: GrowthNode = {
          id: uuidv4(),
          age: (record.age as number) || 6,
          content: {
            title: record.title as string,
            description: record.description as string,
            type: (record.type as GrowthNode['content']['type']) || 'cultural',
            milestones: (record.milestones as string[]) || [],
            smartTags: (record.tags as string[]) || []
          },
          metadata: {
