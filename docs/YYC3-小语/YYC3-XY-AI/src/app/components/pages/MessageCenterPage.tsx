...
export const MessageCenterPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen pb-20">
      {/* Header */}
      <div className={`${isCyber ? 'cyber-header' : isAurora ? 'aurora-header' : 'glass-header'} sticky top-0 z-10`}>
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${isCyber ? 'bg-cyan-500' : isAurora ? 'bg-emerald-500' : 'bg-blue-500'}`}>
                <Bell className="w-5 h-5 text-white" />
              </div>
              <h1 className={isDark ? 'text-white/90' : 'text-gray-900'}>消息中心</h1>
            </div>
            <button className={`flex items-center gap-1 text-sm ${isCyber ? 'text-cyan-400' : isAurora ? 'text-emerald-400' : 'text-blue-600'}`}>
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="mb-6">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className={isDark ? 'text-white/90' : 'text-gray-900'}>
          ...
          {groupedMessages.homework?.map((message) => (
            <div
              key={message.id}
              className={`${isCyber ? 'cyber-card' : isAurora ? 'aurora-card' : 'glass-card'} p-4 mb-3 transition-all cursor-pointer ${
                message.read ? '' : isCyber ? 'border-2 border-cyan-400/40' : isAurora ? 'border-2 border-emerald-400/40' : 'border-2 border-yellow-300'
              }`}
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <div className="flex items-start justify-between gap-2 mb-1">
                    <h4 className={`flex-1 ${isDark ? 'text-white/85' : message.categoryColor}`}>
                      {message.title}
                    </h4>
                    <span className={`text-xs flex-shrink-0 ${isDark ? 'text-white/40' : 'text-gray-500'}`}>
                      {message.timestamp.toLocaleString()}
                    </span>
                  </div>
                  <p className={`text-sm ${isDark ? 'text-white/60' : 'text-gray-600'}`}>{message.content}</p>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="mb-6">
          ...
          {groupedMessages.system?.map((message) => (
            <div
              key={message.id}
              className={`${isCyber ? 'cyber-card' : isAurora ? 'aurora-card' : 'glass-card'} p-4 mb-3 transition-all cursor-pointer ${
                message.read ? '' : isCyber ? 'border-2 border-cyan-400/40' : isAurora ? 'border-2 border-emerald-400/40' : 'border-2 border-purple-300'
              }`}
            >
              <div className="flex items-start gap-3">
                <div className="text-2xl flex-shrink-0">{message.icon}</div>
