...
import type { GrowthNode, GrowthTimeline, GrowthStatistics } from '../../../types/growth';
...
const GrowthTreePage: React.FC<{ onNavigate?: (page: string, data?: Record<string, unknown>) => void }> = ({ onNavigate }) => {
  ...
  const [growthTree, setGrowthTree] = useState<GrowthNode[]>([]);
  ...
  const [selectedNode, setSelectedNode] = useState<GrowthNode | null>(null);
  const [showRecordForm, setShowRecordForm] = useState(false);
  const [treeData, setTreeData] = useState<DataNode[]>([]);
  const [aiInsights, setAiInsights] = useState<{
    summary: string;
    trends: { area: string; trend: string; confidence: number }[];
  ...
  const buildTreeStructure = (nodes: GrowthNode[], phases: GrowthTimeline['phases']) => {
    // 按年龄段分组
    const ageGroups: Record<number, GrowthNode[]> = {};
  ...
  const generateAIGrowthInsights = async (_tree: GrowthNode[], _timeline: GrowthTimeline) => {
    // 模拟AI分析结果
    return {
  ...
  const handleNodeClick = (node: GrowthNode) => {
  ...
  const handleRecordSubmit = async (formValues: Record<string, unknown>) => {
    try {
      const record: Partial<GrowthNode> = {
        ...
        content: {
          ...
          type: formValues.type as GrowthNode['content']['type'],
          category: 'general',
          milestones: [],
          attachments: [],
          culturalElements: [],
    ...
    return (
      <Card title="成长领域分布" className="areas-card">
        <div className="areas-list">
          {statistics.growthAreas.map((area, index: number) => (
  ...
  return (
    <ConfigProvider
      theme={{
        ...
        token: {
          colorPrimary: currentTheme === 'cyberpunk' ? '#00f0ff' : currentTheme === 'aurora' ? '#00ff87' : '#10b981',
        },
      }}
    >
