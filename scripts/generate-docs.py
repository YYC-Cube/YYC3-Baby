...
# @Desc: YYC3-XYAI 全文档架构一键生成脚本 | 生成根目录docs/ | 无任何错误
import os
import sys

# ===================== 核心配置区(固定不变) =====================
DOC_ROOT = "docs"  # 根目录固定为docs/
CREATE_DATE = "2025-12-29"
VERSION = "v1.0.0"
STATUS = "published"
ENCODING = "utf-8"
...
MAIN_MD_TEMPLATE = """---
@file: {FILE_NAME}
@description: YYC3-XYAI {DESCRIPTION}
@author: YanYuCloudCube Team
@version: {VERSION}
@created: {CREATE_DATE}
@updated: {CREATE_DATE}
...
本文档详细描述YYC3-YYC3-XYAI-{DOC_CATEGORY}-{DOC_NAME}相关内容，确保项目按照YYC³标准规范进行开发和实施。
...
README_MD_TEMPLATE = """---
@file: YYC3-XYAI-{DOC_CATEGORY}-文档索引.md
@description: YYC3-XYAI {DESCRIPTION}
@author: YYC³
@version: {VERSION}
@created: {CREATE_DATE}
...
本文档详细描述YYC3-XYAI项目的文档索引模版相关内容，确保项目按照YYC³标准规范进行开发和实施。
...
YYC³(YanYuCloudCube)小语项目是一个基于「五高五标五化」理念的智能化应用系统，致力于提供高质量、高可用、高安全的成长守护体系。
...
PROJECT_STRUCT = {
    "需求规划": [
        ...
        ("005", "教育目标与课程体系设计", "智能教育核心教学目标拆解与课程体系搭建，匹配业务核心诉求", "[需求规划],[教育体系],[课程设计]"),
    ...
    "架构设计": [
        ("021", "系统架构设计", "全系统整体架构设计，包含分层、分模块、部署架构的核心设计方案", "[架构设计],[系统架构],[整体设计]"),
        ("022", "数据架构设计", "项目全数据流转、存储、处理的架构设计，明确数据链路与数据规范", "[架构设计],[数据架构],[数据流转]"),
        ("023", "技术选型报告", "全栈技术栈选型依据与对比分析，匹配五高五标五化的核心设计原则", "[架构设计],[技术选型],[技术评估]"),
        ("024", "数据库设计文档", "数据库表结构、关系、索引、分库分表的详细设计，保障数据存储规范", "[架构设计],[数据库],[表结构设计]"),
        ("025", "安全架构设计", "系统全维度安全防护设计，包含数据安全、接口安全、权限安全等", "[架构设计],[安全架构],[风险防护]"),
        ("026", "微服务拆分设计", "微服务模块拆分原则与边界定义，明确服务间通信与协作规则", "[架构设计],[微服务],[服务拆分]"),
        ("027", "缓存架构设计", "缓存策略、缓存层级、缓存失效处理的设计方案，提升系统性能", "[架构设计],[缓存设计],[性能优化]"),
        ("028", "分布式链路设计", "分布式系统链路追踪、调用链设计，保障系统可观测性与问题排查", "[架构设计],[分布式],[链路追踪]"),
        ("029", "高并发限流设计", "高并发场景下的限流、熔断、降级方案，保障系统高可用与稳定性", "[架构设计],[高并发],[限流熔断]"),
        ("030", "多环境架构适配", "开发、测试、预生产、生产环境的架构适配与配置规范", "[架构设计],[环境管理],[多环境适配]"),
    ...
    "API文档": [ # 多维度细分 ✅ 20个编号位
        ...
        ("059", "业务域-学生端接口手册", "学生端所有业务接口的详细定义，包含入参、出参、调用示例", "[API接口],[业务域],[学生端]"),
        ("060", "业务域-教师端接口手册", "教师端所有业务接口的详细定义，包含入参、出参、调用示例", "[API接口],[业务域],[教师端]"),
        ("061", "业务域-家长端接口手册", "家长端所有业务接口的详细定义，包含入参、出参、调用示例", "[API接口],[业务域],[家长端]"),
        ("062", "业务域-管理员端接口手册", "管理员端所有业务接口的详细定义，包含入参、出参、调用示例", "[API接口],[业务域],[管理员端]"),
        ("063", "业务域-教务课程接口手册", "教务与课程管理相关接口的详细定义，包含课程、排班、考勤等", "[API接口],[业务域],[教务课程]"),
        ("064", "业务域-数据分析接口手册", "教育数据统计、分析、报表相关接口的详细定义与调用规范", "[API接口],[业务域],[数据分析]"),
    ...
    "类型定义": [ # 多维度细分 ✅ 20个编号位
        ...
        ("085", "业务模块-课程体系类型约束", "课程、章节、课时的类型定义与业务规则的完整规范", "[类型定义],[业务模块],[课程]"),
        ("086", "业务模块-测评数据类型文档", "学生测评、成绩、报告的类型定义与数据规则的完整规范", "[类型定义],[业务模块],[测评]"),
    ...
    "测试验证": [
        ...
        ("120", "用户验收测试", "用户参与的验收测试结果，确认系统是否满足业务需求", "[测试验证],[验收测试],[用户确认]"),
        ...
        ("124", "接口自动化测试", "接口自动化测试的执行结果、通过率、问题记录", "[测试验证],[接口测试],[自动化]"),
        ("125", "回归测试用例", "缺陷修复后的回归测试用例，保障修复不引入新问题", "[测试验证],[回归测试],[缺陷修复]"),
        ("126", "安全渗透测试", "专业安全渗透测试的结果与漏洞整改方案", "[测试验证],[渗透测试],[安全防护]"),
        ("127", "测试环境配置", "测试环境的搭建、配置、维护的完整指南", "[测试验证],[环境配置],[测试环境]"),
    ...
    "部署发布": [
        ("136", "部署计划文档", "系统上线部署的整体规划，包含环境、流程、责任人、回滚预案", "[部署发布],[部署计划],[上线规划]"),
        ("137", "发布说明文档", "版本发布的内容、变更点、兼容说明、注意事项的完整文档", "[部署发布],[发布说明],[版本更新]"),
        ("138", "环境配置文档", "生产、预生产环境的服务器、数据库、中间件的配置规范", "[部署发布],[环境配置],[生产环境]"),
        ("139", "回滚方案文档", "版本发布失败后的回滚流程、步骤、验证标准的完整预案", "[部署发布],[回滚方案],[故障恢复]"),
        ...
        ("144", "云服务器配置", "云服务器的选型、配置、安全组、监控的完整规范", "[部署发布],[云服务],[服务器]"),
    ...
    "运维阶段": [
        ("151", "运维手册文档", "系统日常运维的完整指南，包含监控、告警、故障处理、维护", "[运维阶段],[运维手册],[日常维护]"),
    ...
    "综合支撑": [
        ("181", "用户手册-学生端", "学生端产品的使用指南、操作流程、常见问题的完整手册", "[综合支撑],[用户手册],[学生端]"),
        ("182", "用户手册-教师端", "教师端产品的使用指南、操作流程、常见问题的完整手册", "[综合支撑],[用户手册],[教师端]"),
        ("183", "用户手册-家长端", "家长端产品的使用指南、操作流程、常见问题的完整手册", "[综合支撑],[用户手册],[家长端]"),
        ("184", "用户手册-管理员手册", "管理员端产品的使用指南、权限配置、运维操作的完整手册", "[综合支撑],[用户手册],[管理员端]"),
        ("185", "培训文档-全角色培训课件", "面向学生、教师、家长、管理员的全角色培训课件与教程", "[综合支撑],[培训文档],[全角色]"),
        ...
        ("193", "扩展文档-教育数据分析报告", "产品教育数据的统计、分析、价值挖掘的完整报告", "[综合支撑],[扩展文档],[数据分析]"),
...
def generate_project():
    """一键生成全部文档架构"""
    print("✅ 开始生成 YYC3-XYAI 文档架构 | 根目录: {} | 编码: {}".format(DOC_ROOT, ENCODING))
    print("="*80)
    # 创建根目录
    create_dir(DOC_ROOT)
    ...
    for doc_category, doc_list in PROJECT_STRUCT.items():
        # 创建分类目录
        cate_dir = os.path.join(DOC_ROOT, "YYC3-XYAI-{}".format(doc_category))
        create_dir(cate_dir)
        print("📂 创建目录: {}".format(cate_dir))

        # 生成该目录下的所有文档
        for doc_no, doc_name, doc_desc, doc_tags in doc_list:
            # 拼接文件名和标题
            file_name = "{}-YYC3-XYAI-{}-{}.md".format(doc_no, doc_category, doc_name)
            file_path = os.path.join(cate_dir, file_name)
            title = "{}-YYC3-XYAI-{}-{}".format(doc_no, doc_category, doc_name)
        ...
        print("  📑 生成索引: {}".format(readme_name))
        print("-"*80)

    print("✅ 生成完成！所有文档已生成至: {}".format(os.path.abspath(DOC_ROOT)))
    print("✅ 文档总数: 200个(含预留位) + {}个README索引文档".format(len(PROJECT_STRUCT)))
    print("✅ 所有文档严格遵循 YYC3-XYAI 命名规范与模板标准！")
